...
arrBox["dvUser"] = "<img style='margin:2px;' src='"+imgpath+"images/zhuce1.gif'/>&nbsp;1-20个字母、数字、汉字、下划线。";
...
arrWrong["dvUser"] = "<img style='margin:3px;' src='"+imgpath+"images/zhuce2.gif'/>&nbsp;1-20个字母、数字、汉字、下划线。";
...
$(document).ready(
function() {
	$('#dvRec').html('<font style="color:red">填写推荐人用户名，没有可不填。</font>');
    Init();
    $("#txtUser").focus();
...
function RegSubmit(ctrl) {
	...
	$.ajax({
		url: curpath+"/regtemp/",
		data: {"txtEmail": $("#txtEmail").val(),"txtUser": $("#txtUser").val(),"txtPwd": $("#txtPwd").val(),"sVerCode": $("#txtCode").val(),"txtRec": $("#txtRec").val(),"is_transfer":$("input[name='user_type']:checked").val()},//,"is_transfer":$("input[name='user_type']:checked").val()
		//timeout: 8000,
		cache: false,
		type: "post",
		dataType: "json",
