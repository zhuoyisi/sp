...
class MemberdataAction extends ACommonAction{
	...
	public function doEdit(){
        ...
        if ($result = M('member_data_info')->save($vd)) { //保存成功
			...
			alogs("Memberdata",0,1,'成功执行了会员第'.$vd['id'].'号资料('.$vd[data_name].')的审核操作！');
            //成功提示
            $this->assign('jumpUrl');
            $this->success(L('修改成功'));
        } else {
			alogs("Memberdata",0,0,'执行会员第'.$vd['id'].'号资料('.$vd[data_name].')的审核操作失败！');
            //失败提示
            $this->error(L('修改失败'));
        }
	}
	...
	public function doupload(){
		...
		$result = alogs("Memberdata",0,1,'成功执行了上传会员资料的操作！');
