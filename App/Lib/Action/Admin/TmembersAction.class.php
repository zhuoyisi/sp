    ...
    */
    ...
    {	
		...
		if($newid){
			...
			alogs("Tmembers",$newid,0,'执行流转会员的添加操作失败！');
			$this->error("添加失败，请重试");
		}
    }
    ...
    {	
		$pre = C('DB_PREFIX');
		$id = intval($_GET['id']);
		$vo = M('members m')->field("m.id,m.user_name,m.reg_time,m.user_email,mf.info,mf.address,mf.real_name")->join("{$pre}member_info mf ON m.id=mf.uid")->where("m.id={$id}")->find();
		$this->assign("vo",$vo);
        $this->display();
    }
