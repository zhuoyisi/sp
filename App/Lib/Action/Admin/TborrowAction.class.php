...
class TborrowAction extends ACommonAction
{
	...
	public function _addFilter()
	{
		...
		unset($repayment_type[3], $repayment_type[6]);
	...
	public function doAdd( )
	{
		...
		$model->startTrans();
		$model->total = $_POST['borrow_duration'];
		//$model->repayment_type = 5;
		
		if(strtolower($_POST['is_day'])=='yes'){
			$model->repayment_type = 1;
		}else{
		...
		if($model->repayment_type == 1){
			$model->deadline = time() + $_POST['borrow_duration'] * 24 * 3600;
		}else{
		    $model->deadline = time() + $_POST['borrow_duration'] * 30 * 24 * 3600;
		}
		...
		if ($result && $result2) { //保存成功
			...
			alogs("Tborrow",$result,1,'成功执行了企业直投信息的添加操作！');
		  //成功提示
			$this->assign('jumpUrl', __URL__);
			$this->success(L('新增成功'));
		}else{
			alogs("Tborrow",$result,0,'执行企业直投信息的添加操作失败！');
			$model->rollback();
			//失败提示
			$this->error(L('新增失败'));
		}
    ...
    public function doEdit() {
        ...
        //保存当前数据对象
		//$model->repayment_type = 5;
		$model->repayment_type = intval($_POST['borrow_max']);
		$model->borrow_max = intval($_POST['borrow_max']);
        ...
        if ($result || $result2) { //保存成功
			$model->commit();
			alogs("Tborrow",0,1,'成功执行了企业直投信息的修改操作！');
          //成功提示
            $this->assign('jumpUrl', __URL__);
            $this->success(L('修改成功'));
        } else {
			alogs("Tborrow",0,0,'执行企业直投信息的修改操作失败！');
			$model->rollback();
            //失败提示
            $this->error(L('修改失败'));
        }
