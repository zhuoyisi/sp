...
where('borrow_status=0')->count('id');//初审
		...
		$map=array();
		$map['borrow_status'] = 6;
		$map['deadline'] = array('between',strtotime(date("Y-m-d 00:00:00",time())).",".strtotime("+7 days",strtotime(date("Y-m-d 23:59:59",time()))));
		$count_sql = M('borrow_info')->field('id')->where($map)->group("id")->buildSql();
		$newsql = M()->query("select count(*) as tc from {$count_sql} as t");
		$count = $newsql[0]['tc'];
		$row['will_repay'] = $count;//一周内到期
		$this->assign("row",$row);
