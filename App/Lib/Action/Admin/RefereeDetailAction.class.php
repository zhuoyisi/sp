...
class RefereeDetailAction extends ACommonAction
{
    ...
    */
	...
	#	$count = $newxsql[0]['tc'];
		...
		$tfield= ' sum(bi.investor_capital) investor_capital,count(bi.id) total,bi.investor_uid,m.recommend_id,m.id,m.user_name,m.reg_time,mi.real_name,mt.vip_status,mt.phone_status,mt.email_status';
		$tlist = M('transfer_borrow_investor bi')->join("{$this->pre}members m ON m.id = bi.investor_uid")->join("{$this->pre}member_info mi ON mi.uid=m.id")->join("{$this->pre}members_status mt ON mt.uid=m.id")->field($tfield)->where($map)->group('bi.investor_uid')->limit($Lsql)->find();
		//echo  M("transfer_borrow_investor bi")->getLastSql(); 
