...
class MarketAction extends ACommonAction
{
    ...
    public function doGoodEdit(){
        ...
        if($newid){
			alogs("MarketAdd",$newid,1,'成功执行了积分商城商品的添加操作！');
            $this->assign('jumpUrl', __URL__."/goods");
        	$this->success("添加成功");
        }else {
			alogs("MarketAdd",$newid,0,'执行积分商城商品的添加操作失败！');
			$this->error("添加失败");
		}
    }
	...
	public function good_del(){
        ...
        if($del){
			alogs("MarketDel",0,1,'成功执行了积分商城商品的删除操作！');
            $this->assign('jumpUrl', __URL__."/goods");
        	$this->success("删除成功");
        }else{
			alogs("MarketDel",0,0,'执行积分商城商品的删除操作失败！');
			$this->error("删除失败");
		}
    }
...
//      $data['add_time'] = time();
        ...
        if($newid){
			$model->commit();
			alogs("Market_lottery_Add",$newid,1,'成功执行了积分商城抽奖奖品的添加操作！');
            $this->assign('jumpUrl', __URL__."/lottery");
        	$this->success("添加成功");
        }else {
			$model->rollback();
			alogs("Market_lottery_Add",$newid,0,'执行积分商城抽奖奖品的添加操作失败！');
			$this->error("添加失败");
		}
    }
	...
	public function lottery_del(){
        ...
        if($del){
			alogs("Market_lottery_Del",0,1,'成功执行了积分商城抽奖奖品的删除操作！');
            $this->assign('jumpUrl', __URL__."/goods");
        	$this->success("删除成功");
        }else{
			alogs("Market_lottery_Del",0,0,'执行积分商城抽奖奖品的删除操作失败！');
			$this->error("删除失败");
		}
    }
	////////////////////////积分抽奖结束////////////////////////
	...
	public function doEdit() {
		...
		if ($model->create()){
		    ...
		    if($result = $model->save()){
				alogs("MarketPinglun",0,1,'成功执行了积分商城商品评论的回复操作！');
		        $this->assign("jumpUrl","/admin/market/comment");
			    $this->success("修改成功");
		    } else {
				alogs("MarketPinglun",0,0,'执行积分商城商品评论的回复操作失败！');
		        $this->error("修改失败");
		    }
		}else {
		    $this->error($model->getError());
