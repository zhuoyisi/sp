...
class PaylogAction extends ACommonAction
{
    ...
    */
	...
	public function _listFilter($list){
	 	$listType = C('PAYLOG_TYPE');
	 	$payType = array('off'=>'线下充值','sumapay'=>'丰付支付','baofoo'=>'宝付','easypay'=>'易生支付','ecpss'=>'汇潮支付','ips'=>'环迅支付','chinabank'=>'网银在线','tenpay'=>'财付通','gfb'=>'国付宝','allinpay'=>'通联支付','sina'=>'新浪支付','cmpay'=>'中国移动支付','95epay'=>'双乾支付');
		...
		foreach($list as $key=>$v){
			...
			$v['rname'] = M("members m")->join("lzh_member_info mi ON m.id=mi.uid")->getFieldById($v['uid'],'mi.real_name');
			$v['cname'] = M("members")->getFieldById($v['uid'],'customer_name');
			$v['way'] = $payType[$v['way']];
			$row[$key]=$v;
		}
		return $row;
    ...
    */
    public function paylogonline()
    {
		...
		$this->assign('type_list',$listType);
		$field= 'id,uid,status,money,add_time,tran_id,way,off_bank,off_way,deal_user';
		$map['way']=array("in",'gfb,ips,chinabank,baofoo,shengpay,tenpay,ecpss,easypay,cmpay,allinpay,sina,sumapay,95epay');
		$this->_list(D('Paylog'),$field,$map,'id','DESC',$xtime);
        $this->display();
    }
