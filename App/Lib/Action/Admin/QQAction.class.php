...
{   
    ...
    public function addqq()
    {
		...
		if(!isset($_POST['fid'])){
		    $newid = M('qq')->add($data);
			if(!$newid>0){
			   alogs("QQ",$newid,0,'执行客服QQ的添加操作失败！');
		       $this->error('添加失败，请确认填入数据正确');
			   exit;
			}
			alogs("QQ",$newid,1,'执行了客服QQ的添加操作！');
			...
			if(!$newid>0){
				alogs("QQ",$newid,0,'执行客服QQ的编辑操作失败！');
			    $this->error('编辑失败，请确认填入数据正确');
				exit;
			}
			alogs("QQ",$newid,1,'执行了客服QQ的编辑操作！');
			$this->assign('jumpUrl',U('/admin/QQ/index/'));
	...
	public function addqun()
    {
		...
		if(!isset($_POST['fid'])){
		    $newid = M('qq')->add($data);
			if(!$newid>0){
				alogs("QQ",$newid,0,'执行客服QQ群的添加操作失败！');
		       $this->error('添加失败，请确认填入数据正确');
			   exit;
			}
			alogs("QQ",$newid,1,'执行了客服QQ群的添加操作！');
			...
			$newid = M('qq')->save($data);
			if(!$newid>0){
				alogs("QQ",$newid,0,'执行客服QQ群的编辑操作失败！');
			    $this->error('编辑失败，请确认填入数据正确');
				exit;
			}
			alogs("QQ",$newid,1,'执行了客服QQ群的编辑操作！');
			$this->assign('jumpUrl',U('/admin/QQ/qun/'));
	...
	public function addtel()
    {
		...
		if(!isset($_POST['fid'])){
		    $newid = M('qq')->add($data);
			if(!$newid>0){
				alogs("QQ",$newid,0,'执行客服电话的添加操作失败！');
		       $this->error('添加失败，请确认填入数据正确');
			   exit;
			}
			alogs("QQ",$newid,1,'执行了客服电话的添加操作！');
			...
			$newid = M('qq')->save($data);
			if(!$newid>0){
				alogs("QQ",$newid,0,'执行客服电话的编辑操作失败！');
			    $this->error('编辑失败，请确认填入数据正确');
				exit;
			}
			alogs("QQ",$newid,1,'执行了客服电话的编辑操作！');
			$this->assign('jumpUrl',U('/admin/QQ/tel/'));
