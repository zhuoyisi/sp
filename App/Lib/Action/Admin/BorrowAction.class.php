...
class BorrowAction extends ACommonAction
{
    ...
    */
    public function waitverify()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.updata,b.borrow_money,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,b.is_tuijian,b.money_collect,b.first_verify_time,b.second_verify_time,b.borrow_interest,b.deadline';
    ...
    public function waitverify2()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.full_time,m.user_name,m.id mid,b.is_tuijian,b.money_collect,b.first_verify_time,b.second_verify_time,b.add_time,b.borrow_interest,b.deadline';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function waitmoney()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,b.is_tuijian,b.has_borrow,b.money_collect,b.first_verify_time,b.second_verify_time,b.borrow_interest,b.deadline';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function repaymenting()
    {
		...
		$field= 'm.id as mid,m.customer_name,b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,sum(d.interest) borrow_interest,(b.borrow_money+sum(d.interest))benxi,b.repayment_money,b.repayment_interest,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.deadline,m.user_name,m.user_phone,b.is_tuijian,b.money_collect,b.first_verify_time,b.second_verify_time,b.add_time';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->join("{$this->pre}investor_detail d ON d.borrow_id=b.id")->where($map)->group('b.id')->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    {//暂时未处理
		...
		$field= 'id,borrow_name,borrow_uid,borrow_duration,borrow_type,borrow_money,borrow_fee,repayment_money,updata,borrow_interest_rate,repayment_type,deadline,first_verify_time,second_verify_time,add_time,borrow_interest';
	...
	public function unfinish(){
		...
		$field= 'b.id,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.deadline,m.id mid,m.user_name,v.deal_user_2,v.deal_time_2,v.deal_info_2,b.first_verify_time,b.second_verify_time,b.add_time,b.borrow_interest';
    ...
    public function done()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.repayment_money,b.deadline,m.id mid,m.user_name,b.first_verify_time,b.second_verify_time,b.add_time,b.borrow_interest,b.repayment_interest,(b.repayment_money+b.repayment_interest) benxi';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id ASC")->select();
		$list = $this->_listFilter($list);
    ...
    public function fail()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,v.deal_user,v.deal_time,m.id mid,v.deal_info,b.first_verify_time,b.second_verify_time,b.borrow_interest,b.deadline';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->join("{$this->pre}borrow_verify v ON b.id=v.borrow_id")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
    ...
    public function fail2()
    {
		...
		$field= 'b.id,b.borrow_name,b.borrow_status,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.updata,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,b.add_time,m.user_name,m.id mid,v.deal_user_2,v.deal_time_2,v.deal_info_2,b.first_verify_time,b.second_verify_time,b.borrow_interest,b.deadline';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->join("{$this->pre}borrow_verify v ON b.id=v.borrow_id")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		$list = $this->_listFilter($list);
	...
	public function sRepayment(){
		...
		if($res===true){
			alogs("Repay",0,1,'网站代还成功！');
			$this->success("代还成功");
		}elseif(!empty($res)){
			$this->error($res);
		}else{
			alogs("Repay",0,0,'网站代还出错！');
			$this->error("代还出错，请重试");
		}
	}
	...
	public function doEditWaitverify2(){
        ...
        if ($result = $m->save()) { //保存成功
			...
			alogs("borrowApproved",$result,1,'复审操作成功！');
            //成功提示
            $this->assign('jumpUrl', __URL__."/".session('listaction'));
            $this->success(L('修改成功'));
        } else {
			alogs("borrowApproved",$result,0,'复审操作失败！');
            //失败提示
            $this->error(L('修改失败'));
		}	
	}
	 ...
	 public function doweek()
    {
		...
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,b.borrow_fee,b.borrow_interest_rate,b.repayment_type,d.deadline,b.first_verify_time,b.second_verify_time,b.add_time,sum(d.interest) borrow_interest,(b.borrow_money+sum(d.interest)) benxi';
		$list = $borrow->field($field)->join($join)->where($map)->group('b.id')->order("b.deadline ASC")->limit($Lsql)->select();
		if($list){
			foreach ($list as &$key) {
	...
	public function dowaitMoneyComplete(){
		...
		if($result) {
			alogs("dowaitMoneyComplete",0,1,'人工处理满标但未进入复审列表的数据操作成功！');
			$this->success("处理成功");
			$this->assign('jumpUrl', __URL__."/".session('listaction'));
		}else{
			alogs("dowaitMoneyComplete",0,0,'人工处理满标但未进入复审列表的数据操作失败！');
			$this->error("处理失败");
			$this->assign('jumpUrl', __URL__."/".session('listaction'));
		}
	}
	 ...
	 public function doinvest()
    {
		...
		$field= 'bi.id bid,b.id,bi.investor_capital,bi.investor_interest,(bi.investor_capital+bi.investor_interest) benxi,bi.invest_fee,bi.add_time,bi.is_auto,m.user_name,m.id mid,m.user_phone,b.borrow_duration,b.repayment_type,m.customer_name,b.borrow_type,b.borrow_name,b.first_verify_time,b.second_verify_time,md.real_name';
		$list = M('borrow_investor bi')->field($field)->join("{$this->pre}members m ON m.id=bi.investor_uid")->join("{$this->pre}member_info md ON md.uid=bi.investor_uid")->join("{$this->pre}borrow_info b ON b.id=bi.borrow_id")->where($map)->limit($Lsql)->order("bi.id DESC")->select();
		$list = $this->_listFilter($list);
		...
		$field= 'b.id,b.borrow_name,b.borrow_uid,b.borrow_duration,b.borrow_type,b.borrow_money,m.user_name,m.id mid,b.is_tuijian,b.has_borrow,b.money_collect,b.borrow_min,b.first_verify_time,b.second_verify_time,b.borrow_interest,b.deadline';
		$list = M('borrow_info b')->field($field)->join("{$this->pre}members m ON m.id=b.borrow_uid")->where($map)->limit($Lsql)->order("b.id DESC")->select();
		
		$list = $this->_listFilter($list);
		$vo = M('members')->field("id,user_name")->select();
		$userlist = array();
		if(is_array($vo)){
			foreach($vo as $key => $v){
	...
	public function doMoneyComplete(){
		...
		if(($vm['borrow_status']!=2)){
			...
			if($done===true) {
				alogs("doMoneyComplete",0,1,'人工处理低于最小投标金额无法正常满标的数据操作成功！');
