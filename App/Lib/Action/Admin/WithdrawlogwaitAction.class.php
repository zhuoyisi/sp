...
class WithdrawlogwaitAction extends ACommonAction
{
    ...
    */
    ...
    {$map=array();
		...
		$field= 'w.*,m.user_name,m.customer_name,mi.real_name,mb.bank_num,mb.bank_name,mb.bank_address,w.id,w.uid,(mm.account_money+mm.back_money) all_money';
		$list = M('member_withdraw w')->field($field)->join("lzh_members m ON w.uid=m.id")->join("lzh_member_info mi ON w.uid=mi.uid")->join("lzh_member_banks mb ON w.uid=mb.uid")->join("lzh_member_money mm on w.uid = mm.uid")->where($map)->order(' w.id DESC ')->limit($Lsql)->select();
