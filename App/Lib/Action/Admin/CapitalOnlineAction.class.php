...
class CapitalOnlineAction extends ACommonAction
{
    ...
    */
    public function charge()
    {
		...
		$field= 'p.*,m.user_name,m.customer_name,mi.real_name';
		$list = M('member_payonline p')->field($field)->join("{$this->pre}members m ON p.uid=m.id")->join("lzh_member_info mi ON p.uid=mi.uid")->where($map)->limit($Lsql)->order("p.id DESC")->select();
		
        $this->assign("way", array('off'=>'线下充值','sumapay'=>'丰付支付','baofoo'=>'宝付','easypay'=>'易生支付','ecpss'=>'汇潮支付','ips'=>'环迅支付','chinabank'=>'网银在线','tenpay'=>'财付通','gfb'=>'国付宝','allinpay'=>'通联支付','sina'=>'新浪支付','cmpay'=>'中国移动支付','95epay'=>'双乾支付'));
	...
	public function chargeexport(){
		import("ORG.Io.Excel");
		alogs("Charge",0,1,'执行了会员充值记录列表导出操作！');
		$map=array();
		if($_REQUEST['uid'] && $_REQUEST['uname']){
			$map['p.uid'] = $_REQUEST['uid'];
			$search['uid'] = $map['p.uid'];	
		...
		$field= 'p.*,m.user_name,m.customer_name,mi.real_name';
		$list = M('member_payonline p')->field($field)->join("{$this->pre}members m ON p.uid=m.id")->join("lzh_member_info mi ON p.uid=mi.uid")->where($map)->limit($Lsql)->select();

		$status = C('PAYLOG_TYPE');
		$row=array();
		$row[0]=array('序号','用户ID','用户名','真实姓名','客服','充值金额','充值手续费','充值状态','对账订单号','充值方式','充值时间');
		$i=1;
		foreach($list as $v){
				$row[$i]['i'] = $i;
				$row[$i]['uid'] = $v['id'];
				$row[$i]['card_1'] = $v['user_name'];
				$row[$i]['card_2'] = $v['real_name'];
				$row[$i]['card_3'] = $v['customer_name'];
				$row[$i]['card_4'] = $v['money'];
				$row[$i]['card_5'] = $v['fee'];
				$row[$i]['card_6'] = $status[$v['status']];
				$row[$i]['card_7'] = $v['tran_id'];
				$row[$i]['card_8'] = $v['way'];
				$row[$i]['card_9'] = date("Y-m-d H:i:s",$v['add_time']);
				$i++;
		}
     ...
     public function withdraw()
    {
		...
		$field= 'w.*,m.user_name,m.customer_name,mi.real_name,(mm.account_money+mm.back_money) account_money';
		$list = M('member_withdraw w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->join("lzh_member_money mm on w.uid = mm.uid")->join("lzh_member_info mi ON w.uid=mi.uid")->where($map)->limit($Lsql)->order("w.id DESC")->select();
	...
	public function withdrawexport(){
		import("ORG.Io.Excel");
		alogs("Withdraw",0,1,'执行了会员提现记录列表导出操作！');
		$map=array();
		if($_REQUEST['uid'] && $_REQUEST['uname']){
			$map['w.uid'] = $_REQUEST['uid'];
			$search['uid'] = $map['w.uid'];	
		...
		$row=array();
		$row[0]=array('序号','用户ID','用户名','真实姓名','客服','提现金额','提现手续费','到账金额','提现状态','提现时间','处理时间','处理人');
		$i=1;
		foreach($list as $v){
				$row[$i]['i'] = $i;
				$row[$i]['uid'] = $v['id'];
				...
				$row[$i]['card_2'] = $v['real_name'];
				$row[$i]['card_3'] = $v['customer_name'];
				$row[$i]['card_4'] = $v['withdraw_money'];
				$row[$i]['card_5'] = $v['second_fee'];
				$row[$i]['card_6'] =($v['withdraw_status']==3)?0:$v['success_money'];
				$row[$i]['card_7'] = $status[$v['withdraw_status']];
				$row[$i]['card_8'] = date("Y-m-d H:i:s",$v['add_time']);
				$row[$i]['card_9'] = ($v['deal_time']>0)?date("Y-m-d H:i:s",$v['deal_time']):"未处理";
				$row[$i]['card_10'] = (!empty($v['deal_user']))?$v['deal_user']:'';
				$i++;
		}
