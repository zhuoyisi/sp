...
class CapitalAccountAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		//分页处理
		$pre = $this->pre;
		$field= 'm.id,m.reg_time,m.user_name,mi.real_name,m.customer_name,mm.money_freeze,mm.money_collect,(mm.account_money+mm.back_money) total_money,mm.account_money,mm.back_money';
	...
	public function export(){
		...
		foreach($list as $key=>$v){
			...
			//待付金额
			$borrow = M()->query("select sum(investor_capital-receive_capital) as repayment_money,sum(investor_interest-receive_interest) as repayment_interest from {$pre}borrow_investor where borrow_uid={$uid} AND status=4");
		...
		$row[0]=array('ID','用户名','真实姓名','客服','注册时间','总余额','可用余额','冻结金额','待收本息金额','待收本金金额','待收利息金额','待付本息金额','待付本金金额','待付利息金额','待确认投标','待审核提现+手续费','处理中提现+手续费','累计提现手续费','累计充值手续费','累计提现金额','累计充值金额','累计支付佣金','累计投标奖励','净赚利息','净付利息','管理员操作资金');
		$i=1;
		foreach($list as $v){
				$row[$i]['uid'] = $v['id'];
