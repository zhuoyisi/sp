...
class CapitalDetailAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$field= 'l.id,l.add_time,m.user_name,l.affect_money,l.freeze_money,l.collect_money,(l.account_money+l.back_money) account_money,l.target_uname,l.type,l.info,mi.real_name';
		$order = "l.id DESC";
		$list = M('member_moneylog l')->field($field)->join("{$this->pre}members m ON m.id=l.uid")->join("{$this->pre}member_info mi ON mi.uid=l.uid")->where($map)->limit($Lsql)->order($order)->select();
