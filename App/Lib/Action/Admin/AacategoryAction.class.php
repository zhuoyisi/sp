...
class AacategoryAction extends ACommonAction
{
    ...
    */
	...
	public function doAddMul(){
        ...
        if($newid) {
			alogs("Aacategory",$newid,1,'栏目批量添加成功！');
			$this->success("栏目批量添加成功");
		}else{
			alogs("Aacategory",$newid,0,'栏目批量添加失败！');
			$this->error("添加失败");
		}
	}
	...
	public function _doDelFilter($id){
		...
		if($x>0){
			alogs("Aacategory",0,0,'删除失败,所删除的栏目包含了没有权限删除的栏目！');
		...
		if($n>0){
			alogs("Aacategory",0,0,'删除失败,所删除的栏目包含有子栏目,或者含有系统分类,不能删除！');
