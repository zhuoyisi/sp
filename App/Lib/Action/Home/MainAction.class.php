...
//		
	...
	public function tinvestcheck()
	{
		...
		$pin_pass = $vm['pin_pass'];
		$amoney = floatval($amoney);
		$binfo = M("transfer_borrow_info")->field( "repayment_type,transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,increase_rate,reward_rate,borrow_duration")->find($borrow_id);
		...
		if($month < $min_month || $max_month < $month)
		{
                                                if($binfo['repayment_type']==1){
                                                         ajaxmsg("本标认购期限只能在'".$min_month."天---".$max_month."天'之间", 3);
                                                }else{
	...
	public function tinvestmoney()
	{
		...
		$amoney = $m['account_money']+$m['back_money'];
		$uname = session("u_user_name");
		$binfo = M("transfer_borrow_info")->field( "repayment_type,borrow_uid,borrow_interest_rate,transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,increase_rate,reward_rate,borrow_duration")->find($borrow_id);
