...
class BangzhuAction extends HCommonAction {
	...
	$receive_capitalr=array();
    $receive_capitalr = M('borrow_investor')->where($map2)->SUM('investor_capital') +  M('transfer_borrow_investor')->where($map2)->SUM('investor_capital');
	...
	$total_allr=array();
    $total_allr = M('borrow_investor')->where($map1)->COUNT('DISTINCT investor_uid');
	$totalr_all=array();
    $totalr_allr= M('borrow_investor')->where($map2)->COUNT('DISTINCT investor_uid');
	
	$this->assign("total_all",$total_all);
	$this->assign("totalr_all",$totalr_all);
	$this->assign("receive_capital",$receive_capital);
	$this->assign("receive_capitalr",$receive_capitalr);
	$this->assign("receive_interest",$receive_interest);
	$this->assign("total_allr",$total_allr);
	$this->assign("totalr_allr",$totalr_allr);
	
	$map3['borrow_status'] = array("in","2,4,6,7");
	$map4['borrow_status'] = array("in","6");
	$total=array();
    $total = M('borrow_info')->where($map3)->COUNT('id');
	$totalr=array();
    $totalr = M('borrow_info')->where($map4)->COUNT('id');;
	$this->assign("total",$total);
	$this->assign("totalr",$totalr);
	
	$total_all=array();
    $total_all = M('borrow_info')->where($map3)->COUNT('DISTINCT borrow_uid');
	$totalr_all=array();
    $totalr_all= M('borrow_info')->where($map4)->COUNT('DISTINCT borrow_uid');
	$this->assign("total_all",$total_all);
	$this->assign("totalr_all",$totalr_all);
