...
class FundAction extends HCommonAction {
	...
	public function investcheck()
	{
		...
		$binfo = M("transfer_borrow_info")->field( "transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,increase_rate,reward_rate,borrow_duration,collect_day")->find($borrow_id);
	...
	public function investmoney()
	{
		...
		$amoney = $m['account_money']+$m['back_money'];
		$uname = session("u_user_name");
		$binfo = M("transfer_borrow_info")->field( "borrow_uid,borrow_interest_rate,transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,increase_rate,reward_rate,borrow_duration,collect_day,on_off,online_time")->find($borrow_id);
		$mo = M('members_status')->field("email_status,phone_status")->where("uid={$this->uid}")->find();
		if($binfo['on_off']!=1){
		    $this->error("此标无法投资！");
		}
...
//		}
    ...
    public function investRecord($borrow_id=0)
    {
        ...
        if($_GET['borrow_id']){
            ...
            foreach($list as $k=>$v){
				$string .= "<tr>
                          <td height='25' align='left' valign='middle'>".hidecard($v['user_name'],4)."</td>
                          ...
                          <td align='left' valign='middle'>手动</td>
                          <td align='left' valign='middle'><img src='/Style/H/images/zhangtai.png' /></td></tr>";
