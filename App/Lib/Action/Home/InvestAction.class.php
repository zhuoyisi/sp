...
class InvestAction extends HCommonAction {
    ...
    */
    public function index()
    {
		...
		$vo1 = M('members')->field('id,user_name,user_email,user_pass,is_ban,is_transfer')->where("id={$this->uid}")->find();
    ...
    public function detail(){
		...
		//data_list
		$data_list = M("member_data_info")->field('type,add_time,count(status) as num,sum(deal_credits) as credits,data_name,deal_time,status')->where("uid={$borrowinfo['borrow_uid']}")->group('type')->select();
	...
	public function investcheck(){
		...
		if( $money>$caninvest && ($need-$money)<>0 ){
			$msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您投标{$money}元，将导致最后一次投标最多只能投{$last_money}元，小于最小投标金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>或者投标金额必须<font color='#FF0000'>小于等于{$caninvest}元</font>";
			if($caninvest<$binfo['borrow_min']) $msg = "尊敬的{$uname}，此标还差{$need}元满标,如果您投标{$money}元，将导致最后一次投标最多只能投{$last_money}元，小于最小投标金额{$binfo['borrow_min']}元，所以您本次可以选择<font color='#FF0000'>满标</font>即投标金额必须<font color='#FF0000'>等于{$need}元</font>";
	...
	public function investmoney(){
		...
		if(!empty($binfo1['password'])){
			if(empty($_POST['borrow_pass'])) ajaxmsg("此标是定向标，必须验证投标密码",3);
			else if($binfo1['password']<>md5($_POST['borrow_pass'])) ajaxmsg("投标密码不正确",3);
		}
		////////////////////////////////////待收金额限制 2013-08-26  fan///////////////////
		if($binfo['money_collect']>0){
        ...
        if($_GET['borrow_id']){
           ...
           foreach($list as $k=>$v){
               ...
               $string .= "<tr style='background-color: rgb(255, 255, 255);' class='borrowlist3'>
                   <td width='148' class='txtC'>".hidecard($v['user_name'],4)."</td>
