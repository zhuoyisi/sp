...
class PayAction extends HCommonAction {
	...
	public function baofoo(){
		...
		if ($_SERVER['HTTPS'] != "on") {
			$url = "http://gw.baofoo.com/payindex";
		 }else{
...
//通联支付
	...
	#	print_r($submitdata);exit;
        $this->create( $submitdata,"https://www.sumapay.com/sumapay/unitivepay_bankPayForNoLoginUser" );//丰付收银台
    }
	
		//双乾支付
	public function epay(){
		if ( $this->payConfig['95epay']['enable'] == 0 ){
			exit( "对不起，该支付方式被关闭，暂时不能使用!" );
		}
		$this->getPaydetail();

		$submitdata['MerNo'] = $this->payConfig['95epay']['MerCode'];
		$submitdata['Amount'] = number_format( $this->paydetail['money'], 2, ".", "" );
		$submitdata['BillNo'] =	date( "YmdHis" ).mt_rand( 100000, 999999 );
		$submitdata['NotifyURL'] =	$this->notice_url."?payid=epay";//返回地址
		$submitdata['ReturnURL'] =	$this->return_url."?payid=epay";//返回地址

		$submitdata['PayType'] =	"CSPAY";
		$submitdata['PaymentType'] =	($this->paydetail['bank']!="95EPAY")?$this->paydetail['bank']:"";
		
		$key = $this->payConfig['95epay']['MerKey'];    //<--支付密钥--> 注:此处密钥必须与商家后台里的密钥一致
		$submitdata['MD5info'] = strtoupper(md5("Amount={$submitdata['Amount']}&BillNo={$submitdata['BillNo']}&MerNo={$submitdata['MerNo']}&ReturnURL={$submitdata['ReturnURL']}&".strtoupper(md5($key))));

		unset( $this->paydetail['bank'] );
		//网站记录
		$this->paydetail['fee'] = getfloatvalue( $this->payConfig['95epay']['feerate'] * $this->paydetail['money'] / 100, 2 );
		$this->paydetail['nid'] = $this->createnid( "95epay", $submitdata['BillNo'] );
		$this->paydetail['way'] = "95epay";
		M("member_payonline")->add( $this->paydetail);
	...
	public function payreturn(){
		...
		case "epay" :
				if ( $_REQUEST['Succeed'] == "88" ){
					$nid = $this->createnid( "95epay", $_REQUEST['BillNo'] );
					$key=$this->payConfig['95epay']['MerKey'];
					$signGet = strtoupper(md5("Amount={$_REQUEST['Amount']}&BillNo={$_REQUEST['BillNo']}&MerNo={$_REQUEST['MerNo']}&Succeed={$_REQUEST['Succeed']}&".strtoupper(md5($key))));
					if ( $_REQUEST['MD5info'] == $signGet ){
						$this->success( "充值完成", __APP__."/member/" );
					}else{
						$this->error( "签名不付", __APP__."/member/" );
					...
					$this->error( "充值失败,错误返回码：".$_REQUEST['Succeed'], __APP__."/member/" );
				}
		break;
		}
	}
