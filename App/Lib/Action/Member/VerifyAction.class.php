...
class VerifyAction extends MCommonAction {
    ...
    public function index(){
		...
		$vv = M("member_info mi")->join("{$this->pre}lzh_members m ON m.id=mi.uid")->field('mi.idcard,mi.real_name,m.is_transfer')->find($this->uid);
			$this->assign("vv",$vv);
    ...
    public function saveid(){
		...
		$wheresfz['idcard']=$data['idcard'];
		$infores=M('member_info')->where($wheresfz)->select();
		if($infores){
		ajaxmsg("身份号码重复，请重新输入",0);
		}
		$data['up_time'] = time();
		/////////////////////////
		...
		$id_card=M('member_info')->getFieldByUid($this->uid,'idcard');
	
		$idstatus=M('members_status')->where('uid = '.$this->uid)->select();
		if($idstatus[0]['id_status'] == 3) {
			ajaxmsg("您已提交身份验证！请刷新页面",0);
		} elseif($idstatus[0]['id_status'] == 1){
	...
	public function changephoneact(){
		...
		if($pcode&&$xs==1){
			...
			$result = M("member_info")->where("uid={$this->uid}")->setField('cell_phone',text($_POST['phone']));
			session('temp_phone',NULL);
			if($newid&&$result) ajaxmsg();
			else  ajaxmsg('',0);
		}
		else ajaxmsg('',0);
	}
