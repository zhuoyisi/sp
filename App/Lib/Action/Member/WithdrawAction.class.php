...
class WithdrawAction extends MCommonAction {
	...
	public function validate(){
		...
		$pwd = md5($_POST['pwd']);
		$vo = M('members m')->field('mm.account_money,(mm.account_money+mm.back_money) all_money,mm.money_collect,mm.back_money,m.user_leve,m.time_limit')->join("{$pre}member_money mm on mm.uid = m.id")->where("m.id={$this->uid} AND m.pin_pass='{$pwd}'")->find();
        $borrow_info = M("borrow_info")
                        ->field("sum(borrow_money+borrow_interest+borrow_fee) as borrow, sum(repayment_money+repayment_interest) as also")
                        ->where("borrow_uid = {$this->uid} and borrow_type=4 and borrow_status in (0,2,4,6,8,9,10)")
                        ->find();
        ...
        if($vo['money_collect'] < $dh_money){
            ajaxmsg("存在债权转让标借款".($borrow_info['borrow']+$borrow_info['also'])."元未还，请先还款",2);
        }
