<?php
// 本类由系统自动生成，仅供测试用途
class AgreementingAction extends MCommonAction {
 ...
 public function downfile(){
		...
		$iinfo = M('borrow_investor')->field('id,borrow_id,investor_capital,investor_interest,deadline,investor_uid,add_time')->where("(investor_uid={$this->uid} OR borrow_uid={$this->uid}) AND borrow_id={$borrow_id}")->find();
		$binfo = M('borrow_info')->field('id,repayment_type,borrow_duration,borrow_uid,borrow_type,borrow_use,borrow_money,full_time,add_time,borrow_interest_rate,deadline,second_verify_time,borrow_fee,borrow_name,(borrow_money + borrow_interest) benxi,borrow_info')->find($iinfo['borrow_id']);
		...
		$field= 'bi.id bid,b.id,bi.investor_capital,bi.investor_interest,bi.invest_fee,bi.add_time,bi.is_auto,m.user_name,m.id mid,m.user_phone,b.borrow_duration,b.repayment_type,m.customer_name,b.borrow_type,b.borrow_name,b.first_verify_time,b.second_verify_time';
		$list = M('borrow_investor bi')->field($field)->join("{$this->pre}members m ON m.id=bi.investor_uid")->join("{$this->pre}borrow_info b ON b.id=bi.borrow_id")->where("bi.borrow_id={$borrow_id}")->limit($Lsql)->order("bi.id DESC")->select();
