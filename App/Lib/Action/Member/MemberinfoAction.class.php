...
class MemberinfoAction extends MCommonAction {
	...
	public function editmemberinfo(){
        ...
        if (false === $model->create($savedata)) {
			...
			$json['message'] = "<img width='25' height='24px' style='margin-left:-15px;' src='/Style/ad_right/dui.png'/>"."&nbsp;<a style='font-weight:bold'>修改成功</a>";
			$json['status'] = 1;
			exit(json_encode($json));
        } else {
			$json['message'] ="<img width='25' height='24px' style='margin-left:-10px;' src='/Style/ad_right/cuo.png'/>"."&nbsp;<a style='font-weight:bold'>修改失败或者资料没有改动</a>";
			$json['status'] = 0;
			exit(json_encode($json));
        }
	}
	...
	public function editensure(){
        ...
        if (false === $model->create($savedata)) {
			...
			if($nid) $json['message'] = "<img width='25' height='24px' style='margin-left:-15px;' src='/Style/ad_right/dui.png'/>"."&nbsp;<a style='font-weight:bold'>修改成功</a>";
			else $json['message'] = "<img width='25' height='24px' style='margin-left:-15px;' src='/Style/ad_right/dui.png'/>"."&nbsp;<a style='font-weight:bold'>修改成功</a>";
			$json['status'] = 1;
			exit(json_encode($json));
        } else {
			if($nid) $json['message'] = "<img width='25' height='24px' style='margin-left:-10px;' src='/Style/ad_right/cuo.png'/>"."&nbsp;<a style='font-weight:bold'>修改失败或者资料没有改动</a>";
			else  $json['message'] = "<img width='25' height='24px' style='margin-left:-10px;' src='/Style/ad_right/cuo.png'/>"."&nbsp;<a style='font-weight:bold'>修改失败或者资料没有改动</a>";
			$json['status'] = 0;
			exit(json_encode($json));
        }
	}
	...
	public function editdata(){
		...
		if(!$_FILES){
			...
			$list = $model->field('id,data_url,data_url1,data_url2,data_url3,data_url4,data_name,add_time,status,type,ext,size,deal_info,deal_credits')->where("uid={$this->uid}")->order("type DESC")->limit($Lsql)->select();
			
			$this->assign('to_upload_type', $to_upload_type);   //待上传的类型
			foreach ($list as $key => $val){
				$integration = FS('Webconfig/integration');
				$list[$key]['class_name']=$integration[$val['type']]['description'];
			}
		...
		$savedata['data_url']=substr($_GET['tupian1'],1);
		$savedata['data_url1']=substr($_GET['tupian2'],1);
		$savedata['data_url2']=substr($_GET['tupian3'],1);
		$savedata['data_url3']=substr($_GET['tupian4'],1);
		$savedata['data_url4']=substr($_GET['tupian5'],1);
