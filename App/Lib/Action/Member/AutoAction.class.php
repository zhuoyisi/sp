...
class AutoAction extends MCommonAction {
    ...
    public function auto(){
		...
		if($vo){
			$vocount = M()->query("SELECT count( `id` ) num FROM `lzh_auto_borrow` WHERE `borrow_type` =1 and `is_use`=1
				AND `invest_time` < ( 
				SELECT `invest_time` 
				FROM `lzh_auto_borrow` 
				WHERE borrow_type=1 and `is_use`=1 and uid ={$this->uid})"); 
		}
		...
		if($vot){
			$votcount = M()->query("SELECT count( `id` ) tnum FROM `lzh_auto_borrow` WHERE `borrow_type` =3 and `is_use`=1
				AND `invest_time` < ( 
				SELECT `invest_time` 
				FROM `lzh_auto_borrow` 
				WHERE borrow_type=3 and `is_use`=1 and uid ={$this->uid})"); 
		}
    ...
    public function autolong(){
		...
		//`mxl:autoday`
		$MAXMOONS = 180;
		$vo['is_auto_day'] = ($vo['duration_to'] >= $MAXMOONS) ? 1 : 0;
