       ...
       */
        ...
        */
        public function buy($paypass, $invest_id)
        {
            ...
            if($check_result === 'TRUE'){
                ...
                //转让方增加资金
                $sell_user['account_money'] += $debt_info['transfer_price'];//$debt_info['transfer_price']-$fee;该处不能扣减手续费，因为在转让成功时扣除了手续费，这里扣就重复了
        ...
        */
        private function checkBuy($paypass, $invest_id)
        {
            ...
            $invest = $this->Model->field("transfer_price, sell_uid, valid,status")->where("invest_id = {$invest_id}")->find();
