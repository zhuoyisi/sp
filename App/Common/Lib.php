	...
	$Moneylog->startTrans();
		...
		/////////////////////////////////////////
			case 5://撤消提现
				...
				if(($MM['back_money']+$amoney+$fee)<10){//提现手续费先从回款余额资金池里扣，不够再去充值资金池里减少
					$data['back_money'] = 0;
			...
			case 36://提现通过，处理中
				$data['affect_money'] = $amoney;
				if(($MM['account_money']+$MM['back_money'])>abs($fee)){
					if(($MM['back_money']+$fee)<10){//提现手续费先从回款余额资金池里扣，不够再去充值资金池里减少
...
function getMemberMoneySummary($uid){  
	...
	$money['ljtbjl'] = $list['20']['money']+$list[41]['money']+1;//$toubiaojl;//累计投标奖励
...
function getDurationCount($uid=0){
	...
	$time_1 = array(strtotime("-12 month",strtotime(date("Y-m-d",time())." 00:00:00")),strtotime(date("Y-m-d",time())." 23:59:59"));
	$time_6 = array(strtotime("-60 month",strtotime(date("Y-m-d",time())." 00:00:00")),strtotime(date("Y-m-d",time())." 23:59:59"));
	$row_time_1=array();
	$row_time_2=array();
	$row_time_3=array();
	$row_time_4=array();
...
function getMinfo($uid,$field='m.pin_pass,mm.account_money,mm.back_money'){
	$pre = C('DB_PREFIX');
	$vm = M("members m")->field($field)->join("{$pre}member_info mi on m.id=mi.uid")->join("{$pre}member_money mm ON mm.uid=m.id")->where("m.id={$uid}")->find();
	return $vm;
}
...
"id,borrow_uid,borrow_money,borrow_interest_rate,borrow_duration,repayment_type,transfer_out,transfer_back,transfer_total,per_transfer,is_show,deadline,min_month,reward_rate,increase_rate,borrow_fee,is_jijin")->find($borrow_id);
	...
	if($binfo['is_jijin']==1){		$parm ="定投宝";	}else{		$parm = "企业直投";	}
	$vminfo = getMinfo($uid,'m.user_leve,m.time_limit,mm.account_money,mm.back_money,mm.money_collect');
	if($num<1){
	...
	switch($binfo['repayment_type']){
	    ...
	    case 6://利息复投
			...
			$investinfo['is_auto'] = $_is_auto;
			$investinfo['add_time'] = time();
			$investinfo['deadline'] = time() + $investinfo['transfer_month'] * 30 * 24 * 360;
			$investinfo['reward_money'] = getFloatValue($binfo['reward_rate'] * $money/100, 2);
			$interest_rate = $binfo['borrow_interest_rate'];
		...
		if($invest_info_id && $Tinvest_defail_id){
			...
			$_borraccount = memberMoneyLog($binfo['borrow_uid'],17,$money,"第{$borrow_id}号{$parm}已被认购{$money}元，{$money}元已入帐");
			if(empty($binfo['transfer_out'])){			    $binfo['transfer_out'] = 0;			}
			if((intval($binfo['transfer_out'])+$num)==$binfo['transfer_total']){//如果企业直投被认购完毕，则扣除借款人借款管理费
				$_borrfee = memberMoneyLog($binfo['borrow_uid'],18,-$binfo['borrow_fee'],"第{$borrow_id}号{$parm}被认购完毕，扣除借款管理费{$binfo['borrow_fee']}元");
				if(!$_borrfee) return false;}
			$endTime = strtotime(date("Y-m-d",time())." ".$_P_fee['back_time']);
