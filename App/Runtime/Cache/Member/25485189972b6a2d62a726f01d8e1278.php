...
.tdTitle { text-align: left; padding-left: 10px; font-size: 12px; height: 40; line-height: 40px; vertical-align: middle; width: 100px; font-weight: bold; background-color: #F9F9F9; border: #E9E8E7 solid 1px; }
...
.alertDiv_pin li { margin: 5px 0; list-style-type: decimal; color: #005B9F; padding: 0px; line-height: 20px; list-style-type:circle }
...
<div style="width: 760px;" class="alertDiv_pin">
	<ul style="display: block;">
	<li>请准确填写您的充值金额和帐单流水号并仔细阅读线下充值委托书!</li>
	</ul>
</div>
...
<form action="__APP__/Pay/offline" method="post" name="offlinePay" id="offlinePay">
	<table style="width: 760px; float: left; margin: 0px 20px;
		padding: 0px; border-collapse: collapse; text-align: left;" id="formTb" cellpadding="0" cellspacing="0">
		<tbody>
        <?php if(is_array($bank)): $i = 0; $__LIST__ = $bank;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td class="tdTitle"><input type="radio" name="bank" id="bank" value="<?php echo ($i); ?>" <?php if($i == 1): ?>checked="checked"<?php endif; ?> > <?php echo ($vo["bank"]); ?></td>
            <td class="tdContent">
                开户名：<?php echo ($vo["payee"]); ?>
                账户号：<?php echo ($vo["account"]); ?>
                开户地址：<?php echo ($vo["address"]); ?>
            </td>
		...
		<tr>
            <td class="tdTitle">线下充值委托书：</td>
			 <td class="tdContent"><a style="text-decoration:underline;color:#FF0000;" target="_blank" href="/Public/charge.html">点击查看</a></td>
		</tr>
		 <tr>
            <td class="tdTitle">线下充值备注：</td>
			 <td class="tdContent"><?php echo ($info); ?></td>
		</tr>
	</tbody>
    </table>
</form>
...
<div style="text-align: center; width: 740px; height: 30px; margin: 10px;">
	<img id="btn_set" class="img_button" onclick="ckoffline()" src="__ROOT__/Style/M/images/btnsubupdate.jpg">
</div>

<script type="text/javascript">

function ckoffline(){
if (!confirm("已仔细阅读并同意线下充值委托书所有条款？")) {
