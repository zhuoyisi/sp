...
<div style="margin-top: 20px; overflow: hidden; text-align: left;">
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr class="nodatashowtr">
		...
		<td class="tdContent"><a href="/invest/<?php echo ($vo["id"]); ?>.html" title="<?php echo ($vo["borrow_name"]); ?>"><?php echo ($vo["borrow_name"]); ?></a></td>
		...
		<td class="tdContent">
		<?php if($vo["borrow_type"] == 1): ?><a href="__APP__/member/agreements/downfile?id=<?php echo ($vo["id"]); ?>" target="_blank">合同</a>
            <?php elseif($vo["borrow_type"] == 2): ?>
			<a href="__APP__/member/agreements/downfile?id=<?php echo ($vo["id"]); ?>" target="_blank">合同</a>
            <?php elseif($vo["borrow_type"] == 5): ?>
			<a href="__APP__/member/agreementing/downfile?id=<?php echo ($vo["id"]); ?>" target="_blank">合同</a>
            <?php else: ?>
