...
<!--搜索/筛选会员-->
  ...
  <tr>
	...
	<th class="line_l">投资成交管理费</th>
    <th class="line_l">还款方式</th>
	<th class="line_l">标种类型</th>
  ...
  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["bid"]); ?>">
        ...
        <td>
		<a href="<?php echo (getinvesturl($vo["id"])); ?>" title="<?php echo ($vo["borrow_name"]); ?>" target="_blank"><?php echo (cnsubstr($vo["borrow_name"],12)); ?></a>
		</td>
        <td><?php echo ($vo["investor_capital"]); ?></td>
        <td><?php echo ($vo["investor_interest"]); ?></td>
		<td><?php echo ($vo["benxi"]); ?></td>
        <td><?php echo ($vo["borrow_duration"]); if($vo["repayment_type_num"] == 1): ?>天<?php else: ?>个月<?php endif; ?></td>
        <td><?php echo ($vo["invest_fee"]); ?></td>
        <td><?php echo ($vo["repayment_type"]); ?></td>
		<td><?php echo ($vo["borrow_type"]); ?></td>
		<td><?php echo ($vo["is_auto"]); ?></td>
