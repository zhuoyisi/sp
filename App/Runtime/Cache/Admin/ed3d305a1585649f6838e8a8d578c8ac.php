...
<div class="form2">
	...
	<dl class="lineD"><dt>是否流转会员：</dt><dd><?php $i=0;$___KEY=array ( 0 => '否', 1 => '是', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$vo["is_transfer"]==$k)||("key"=="value"&&$vo["is_transfer"]==$v)){ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_transfer" value="<?php echo ($k); ?>" id="is_transfer_<?php echo ($i); ?>" /><?php } ?><label for="is_transfer_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?><span id="tip_is_transfer" class="tip">流转会员才能在添加企业直投的时候被选中</span></dd></dl>
	<dl class="lineD"><dt>是否冻结：</dt><dd><?php $i=0;$___KEY=array ( 0 => '否', 1 => '是', 2 => '登录次数超限，已冻结', ); foreach($___KEY as $k=>$v){ if(strlen("1key")==1 && $i==0){ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" checked="checked" /><?php }elseif(("key1"=="key1"&&$vo["is_ban"]==$k)||("key"=="value"&&$vo["is_ban"]==$v)){ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" checked="checked" /><?php }else{ ?><input type="radio" name="is_ban" value="<?php echo ($k); ?>" id="is_ban_<?php echo ($i); ?>" /><?php } ?><label for="is_ban_<?php echo ($i); ?>"><?php echo ($v); ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<?php $i++; } ?><span id="tip_is_ban" class="tip">冻结后会员不能登陆，须联系客服处理</span></dd></dl>
	...
	<dl class="lineD"><dt>客服：</dt>
                            <dd>		 
                               <select  id="customer_id" required="required" name="customer_id">
                                <?php if(is_array($kefu)): $i = 0; $__LIST__ = $kefu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vv): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vv["id"]); ?>" <?php if(($vo["customer_id"]) == $vv["id"]): ?>selected="selected"<?php endif; ?> ><?php echo ($vv["user_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>     
                                </select>
                            </dd>  
                        </dl>
	...
	<dl class="lineD"><dt>真实姓名：</dt><dd><?php echo ($vo["real_name"]); ?></dd></dl>
	<dl class="lineD"><dt>身份证号：</dt><dd><?php echo ($vo["idcard"]); ?></dd></dl>
	...
	<dl class="lineD"><dt>地址：</dt><dd><input name="address" id="address"  class="input" type="text" value="<?php echo ($area[$city[0]['province_now']]); ?>(省)<?php echo ($area[$city[0]['city_now']]); ?>(市)<?php echo ($area[$city[0]['area_now']]); ?>(县)" ></dd></dl>
