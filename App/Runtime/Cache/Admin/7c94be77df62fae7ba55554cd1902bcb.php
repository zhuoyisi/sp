...
<div class="so_main">
  ...
  <div id="addAttr_div" style="display:none;">
  	<div class="page_tit">添加团队长 [ <a href="javascript:void(0);" onclick="addAdminUser();">隐藏</a> ]</div>
	
	<div class="form2">
	<form method="post" action="__URL__/addAdmin" onsubmit="return addNewAdmin();" enctype="multipart/form-data">
    <input type="hidden" name="u_group_id" value="25"/>
    ...
    <dl class="lineD">
      ...
      <dd>
        ...
        <span>主要是针对文章管理和客服,其他类型的团队长可以随便选</span>
      </dd>
    </dl>-->
    ...
    <!--<dl class="lineD">
	 ...
	 <dl class="lineD">
      ...
      <dd>
        ...
        <span>团队长密码口令设置后，在后台登录时只有写对口令才能进入后台（可填写文字、字母或数字，如：客服1号）</span>
      </dd>
    </dl>
...
<!--添加团队长-->
  <div class="Toolbar_inbox">
	...
	<a onclick="addAdminUser();" class="btn_a" href="javascript:void(0);">
		<span class="addAttr_action">添加团队长</span>
	</a>
  ...
  <?php if(is_array($admin_list)): $i = 0; $__LIST__ = $admin_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr overstyle='on' id="list_<?php echo ($vo["id"]); ?>">
		...
		<td><?php if($vo["is_ban"] == 1): ?><font color="#FF0000" id="is_ban_<?php echo ($vo['id']); ?>">禁用</font><?php else: ?><font color="#009900" id="is_ban_<?php echo ($vo['id']); ?>">正常</font><?php endif; ?></td>
 		<td><span id="card_<?php echo ($vo['id']); ?>"><?php echo ($vo["card"]); ?></span></td>
 		<td><span id="qq_<?php echo ($vo['id']); ?>"><?php echo ($vo["qq"]); ?></span></td>
 		<td><span id="phone_<?php echo ($vo['id']); ?>"><?php echo ($vo["phone"]); ?></span></td>
		<td style="display:none"><?php if($vo["is_kf"] == 1): ?><font color="#FF0000" id="is_kf_<?php echo ($vo['id']); ?>">是</font><?php else: ?><font color="#009900" id="is_kf_<?php echo ($vo['id']); ?>">否</font><?php endif; ?></td>
		<td><?php echo (date("Y-m-d H:i:s",$vo["add_time"])); ?><span id="word_<?php echo ($vo['id']); ?>" style=" display:none;"><?php echo ($vo["user_word"]); ?></span><div style=" display:none;"><span id="photo_<?php echo ($vo['id']); ?>"><img src="__ROOT__/<?php echo ($vo["photo"]); ?>" width="100" height="100" /></span><span id="sex_<?php echo ($vo['id']); ?>"><?php echo ($vo["sex"]); ?></span><span id="email_<?php echo ($vo['id']); ?>"><?php echo ($vo["email"]); ?></span><span id="province_<?php echo ($vo['id']); ?>"><?php echo ($vo["province"]); ?></span><span id="city_<?php echo ($vo['id']); ?>"><?php echo ($vo["city"]); ?></span><span id="area_<?php echo ($vo['id']); ?>"><?php echo ($vo["area"]); ?></span><span id="address_<?php echo ($vo['id']); ?>"><?php echo ($vo["address"]); ?></span><span id="content_<?php echo ($vo['id']); ?>"><?php echo ($vo["content"]); ?></span></div></td>
